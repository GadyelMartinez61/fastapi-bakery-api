...
DATABASE = get_table(settings.products_table)


class ProductRepo:
    def get(self, branch_id: str, product_id: str) -> dict | None:
        response = DATABASE.get_item(
            Key={"branch_id": branch_id, "product_id": product_id}
        )
    ...
    def delete(self, branch_id: str, product_id: str) -> dict | None:
        item = self.get(branch_id, product_id)
        if not item:
            return None
        try:
            DATABASE.delete_item(Key={"branch_id": branch_id, "product_id": product_id})
        except ClientError:
            return None

        return item

    def update(self, branch_id: str, product_id: str, data: dict) -> dict:
        old_item = self.get(branch_id, product_id)
